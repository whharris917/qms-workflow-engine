...
Structure: NavigationForm (tabs mode, 4 category tabs) → NavigationForm (accordion mode, per-type sections)
...
_CONTAINER_FORMS = NavigationForm(
    key="container-types",
    ...
    steps=[
        _type_entry(
            "sequence",
            "Unified container with four modes: tabs (free access, one visible), chain (gated, auto-advance), sequence (gated, manual nav), accordion (free, all visible with expand/collapse). Faithful projection — hidden content absent from JSON.",
            "All container navigation patterns. Set mode to control behavior.",
            'mode (str): "tabs", "chain", "sequence", or "accordion". default_expanded (bool, optional): Accordion initial state. Steps are added as children.',
            '{"mode": "tabs"}  # or "chain", "sequence", "accordion"',
        ),
        _type_entry(
            "group",
...
_REACTIVE_FORMS = NavigationForm(
    key="reactive-types",
    ...
    eigenforms=[
        InfoForm(
            ...
            text={
                ...
                "Completeness": (
                    ...
                    "when it has a value. A NavigationForm is complete when all its steps are "
        ...
        InfoForm(
            ...
            text={
                "Pages and children": (
                    ...
                    "or containers (NavigationForm in tabs/chain/accordion modes) that hold their own "
                    "children. You build pages by nesting eigenforms to any depth."
                ),
                "Containers": (
                    "Containers give structure: NavigationForm(mode='tabs') shows one child at a time, "
                    "mode='chain' shows children sequentially (wizard), mode='accordion' shows "
                    "collapsible sections, mode='sequence' gates progression. Containers are "
                    "composable — a tabs container can hold a chain, which holds data forms."
                ),
        ...
        InfoForm(
            ...
            text={
                ...
                "Multi-step workflow": (
                    "A multi-step workflow uses NavigationForm (chain or sequence mode) to gate "
                    "progression. Wrap each phase in a GroupForm inside a NavigationForm. "
        ...
        InfoForm(
            ...
            text={
                ...
                "Containers need children": (
                    "Container types (NavigationForm, GroupForm, etc.) "
