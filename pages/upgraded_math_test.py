...
definition = PageForm(key="upgraded-math-test", label="Upgraded Math Test", instruction=(
    ...
    "Each eigenform includes affordances — these are the actions you can take. "
    ...
    NavigationForm(key="test", label="Questions", mode="chain", instruction="Complete each question to proceed.", steps=[
        ...
        CheckboxForm(key="q3", label="Question 3", instruction="Which of the following are prime numbers?",
                     items=["2", "9", "13", "15", "23"]),
        NavigationForm(key="q4", label="Question 4", mode="tabs", instruction="Follow the instructions on each tab. You will need to jump between tabs.", steps=[
        ...
        NavigationForm(key="q5", label="Question 5", mode="chain", instruction="A sequence of clues. Each step unlocks the next.", steps=[
