...
A Python framework for defining interactive forms as data-class trees, persisted to per-instance JSON files. The form definition is a "seed"; binding it to a store produces a runtime instance. Each form — called an Eigenform — is responsible for serializing its own state, rendering its own HTML, dispatching its own actions, and emitting "affordances" (the URLs and POST bodies the client can act on).

Containers (PageForm, NavigationForm, GroupForm, RepeaterForm) compose forms into trees. The composition is declarative; the runtime walks the tree to handle requests.
...
This page itself is one of those trees: a PageForm containing a NavigationForm in tabs mode, with InfoForms holding the prose. The fact that this works at all — that a 600-word essay on the engine's architecture can be expressed as a composition of the engine's own primitives — is a non-trivial existence proof.
"""

_SECTION_2 = """\
The lifecycle in five steps.

1. Discover seeds. Files in pages/*.py export `definition = PageForm(...)`. `discover_pages()` reloads them on every request — effectively hot reload.
...
4. Handle action. A POST routes through `handle_action(path, body)` which walks the tree to find the target eigenform, calls its `_handle()`, mutates the store, and optionally calls `_rebuild()` to refresh the live tree from the updated structure.
...
This is well-defined, but it leaks complexity. The seed-match in step 3 exists because callables (lambdas in ComputedForm.compute, options_fn in DynamicChoiceForm) cannot round-trip through JSON. That is the structural limit of the descriptor-as-truth principle: it is true for scalar configuration, partial for behavior.
"""
...
Affordance flotation. Repeated affordances (Clear, Edit, Batch) bubble from any nesting depth to the PageForm level as parameterized compounds. About 60% reduction in affordance noise on flat pages, more on deep ones.

Clean module boundaries. `engine/` imports from neither `app/` nor `pages/`. No cycles. Containers and leaves share a single `Eigenform` protocol with consistent overrides.
...
Parity test. `tests/test_parity.py` verifies that every affordance URL emitted in `serialize()` appears as a `data-ef-*` attribute in the rendered HTML for all 18 pages. JSON-vs-HTML divergence is caught at CI time.
"""
...
First, HATEOAS-style affordances. Every eigenform serializes its own URLs and POST bodies. The dream is that any client — human browser, AI agent, headless test runner — can read the page state and act on it without prior schema knowledge. The page is its own API.
...
Third, mutable structure. Pages with `mutable_structure=True` expose a builder UI and an action vocabulary (add_eigenform, move_eigenform, group_eigenforms, reparent_eigenform, ungroup_eigenform). Users — or agents — can edit the page tree at runtime. There is no separate "designer mode"; design and execution are the same surface with different affordances.
...
The supporting evidence is real. The Quiz Portal exists as a composition of seven eigenform types and zero JavaScript. The Page Builder lets you assemble pages by drag-and-drop. The eigenform-gallery page documents itself by rendering one of every type. None of this is mocked.
"""

_SECTION_5 = """\
Where the delta lives. Five honest gaps between ambition and delivery.

1. Action dispatch is unstructured. 88 `if action == ...` checks across 20 files. PageForm._handle alone has 11 elif branches. Adding a new action requires finding the right file, deciding undo policy, deciding rebuild policy, and wiring through `handle_action` path resolution. No registry, no decorator, no metadata. Adding the 30th action will continue this organic growth.

2. String-typed sibling references. SwitchForm, VisibilityForm, DynamicChoiceForm, ScoreForm, ComputedForm, ValidationForm, ActionForm all reference siblings via `depends_on="some_key"`. Delete the referenced sibling and dependents silently orphan. The self-describing protocol cannot describe its own inter-form dependencies. No validation, no warning, no graph.
...
PageForm: 1064 lines. TableForm: 1192 lines. NavigationForm: 665 lines. All three mix orchestration (bind, rebuild) with ad-hoc action dispatchers and tree-walking utilities. Splitting along the (orchestration, structural mutation, content mutation, rendering) axes would expose the actual coupling and probably reveal duplications.

NavigationForm is four classes in one. `mode in ("tabs", "chain", "sequence", "accordion")` switches not only display but also value shape (string for tabs/chain/sequence, dict for accordion), navigation rules, and completion semantics. The single-class implementation is DRY but the API surface is unclear; users must remember the mode is a personality dial. Either four subclasses or an explicit Strategy object.

Scope conventions are not consistent. PageForm: children share its `_scope`. NavigationForm and GroupForm: children get `scope=self.key`. RepeaterForm: `scope=self.key/entry_id`. Adding a new container type requires careful thought to avoid silent breakage. There is no `Scope` type and no architecture document explaining the convention.
...
Demo code in production palette. `RubiksCubeForm` (400 lines) and `TableRunner` ship in the registry alongside production primitives. They appear in the builder's type-picker. They belong in `engine/_examples/`.

`MultiForm`, `ListForm`, `RepeaterForm` all express "multiple values" with different mechanisms. The distinctions are real but require explanation. Better naming or a documented sibling-relationship diagram would help.
...
5. Split the megafiles. PageForm and TableForm partitioned by axis (binding / structural mutation / content mutation / rendering / dispatch). Surface the actual seams.
...
7. Move demo forms out of the production registry. RubiksCubeForm and TableRunner go to `engine/_examples/` and stop appearing in builder type-pickers.
...
definition = PageForm(
    ...
    eigenforms=[
        NavigationForm(
            ...
            mode="tabs",
            steps=[
                InfoForm(key="s1", label="1. What this is",
                         text=_SECTION_1),
                InfoForm(key="s2", label="2. The lifecycle",
                         text=_SECTION_2),
                InfoForm(key="s3", label="3. What works well",
                         text=_SECTION_3),
                InfoForm(key="s4", label="4. The aspiration",
                         text=_SECTION_4),
                InfoForm(key="s5", label="5. The delta",
                         text=_SECTION_5),
                InfoForm(key="s6", label="6. Code-quality audit",
                         text=_SECTION_6),
                InfoForm(key="s7", label="7. Recommendations",
                         text=_SECTION_7),
            ],
        ),
    ],
