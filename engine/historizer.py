...
The current version is always the live, interactive component.
...
class Historizer(Component):
    ...
    def has_data(self) -> bool:
        if self.component is None:
            return False
        return self.component.has_data
    ...
    def _child_snapshot(self) -> dict:
        ...
        state = self.component.serialize()
        if state:
    ...
    def _maybe_record(self):
        ...
        if self.component is None or self._store is None:
            return
    ...
    def _serialize_full(self) -> dict:
        ...
        if viewing is not None:
            ...
            state["component"] = entry["snapshot"]
            state["viewing_timestamp"] = entry["timestamp"]
            state["read_only"] = True
        else:
            state["component"] = self.component.serialize() if self.component else None
    ...
    def render_from_data(self, data: dict) -> str:
        ...
        snapshot_html = ""
        if read_only:
            snapshot = data.get("component", {})
            if snapshot:
                snapshot_html = self._render_snapshot(snapshot)
        else:
            if self.component:
                child_html = self.component.render_safely()
