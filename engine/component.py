"""Component — the self-contained unit of workflow interaction.

A component has internal state, serializes itself to JSON, renders
HTML from that JSON, and exposes affordances as POSTable actions.
Each component is a HATEOAS-compliant mini-application.
...
cannot diverge. Affordances are pure data — the component is responsible
for accounting for each one in render_from_data().

Once bound to a store, scope, and url_prefix, a component is fully
...
def render_dependency_line(depends_on, url_prefix: str = "") -> str:
    """Render a 'Depends on:' indicator for sibling-reading components.
...
def _render_error_card(ef, exc: Exception) -> str:
    ...
    Called by Component.render_safely() when render() raises. The card is
    ...
    component continue to render normally.
...
class Component:
    ...
    def children(self) -> list[Component]:
        """Direct child components. Containers override this."""
        return []
    ...
    def bind(self, store: Store, scope: str, url_prefix: str) -> Component:
        """Produce a bound copy of this component. The original is unchanged.
    ...
    def _get_my_descriptor(self) -> dict | None:
        """Return this component's entry in the parent container's __structure.

        Returns None if no parent structure exists or this component isn't
    ...
    def _migrate_legacy_overrides(self):
        ...
        component into the parent's __structure descriptor, then delete them.
    ...
    def has_data(self) -> bool:
        """Whether this component has user-entered data that can be cleared."""
        return self.value is not None

    def _clear_data(self):
        """Remove this component's data from the store."""
    ...
    def _snapshot_edit_state(self) -> dict:
        """Capture current edit state by snapshotting this component's entry
    ...
    def _get_edit_affordances(self) -> list[Affordance]:
        ...
        affs = [
            Affordance(
                ...
                instruction=f"Rename this component. Current label: {self.label}",
            ),
        ]
        ...
        affs.append(
            Affordance(
                ...
                instruction=f"Change the instruction text. Current: {current_instr}" if current_instr else "Set instruction text for this component.",
            ),
        )
        return affs
    ...
    def is_complete(self) -> bool:
        """Whether this component has been completed. Subclasses must implement."""
        raise NotImplementedError

    @property
    def url(self) -> str:
        """The URL for this component's actions."""
        return f"{self._url_prefix}/{self.key}"

    @property
    def uid(self) -> str:
        """Unique DOM ID for this component, scoped to avoid collisions."""
        return f"c-{self._scope}-{self.key}" if self._scope else f"c-{self.key}"
    ...
    def _descriptor_config(self) -> dict:
        ...
        JSON-safe and not part of the base Component protocol. Private
    ...
    def to_descriptor(self) -> dict:
        """Serialize this component's structural description.

        Returns a dict that, combined with the registry and optionally
        a seed component, can reconstruct this component. Containers
    ...
    def affordances(self) -> list[Affordance]:
        ...
        _aff = Affordance(
            ...
            instruction=(
                ...
                "Each entry in 'actions' uses the same body format as this component's other affordances. "
...
# Forms that are containers (transparent wrapper, no card styling in themes)
    ...
    def render(self) -> str:
        """Render this component as HTML, wrapped in a themed container.
        ...
        if self.edit_mode:
            classes.append('component--editing')
        if self.editable:
            classes.append('component--editable')
